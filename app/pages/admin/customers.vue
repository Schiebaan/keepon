...
function getCustomerModuleCount(customerId: string) {
  const custInstallations = getCustomerInstallations(customerId)
  return custInstallations.reduce((sum, inst) => {
    return sum + getInstallationSubscriptions(inst.id).filter(s => s.status === 'active').length
  }, 0)
}

function getCustomerModuleTypes(customerId: string): string[] {
  const custInstallations = getCustomerInstallations(customerId)
  const types = new Set<string>()
  custInstallations.forEach(inst => {
    getInstallationSubscriptions(inst.id)
      .filter(s => s.status === 'active')
      .forEach(s => {
        const t = s.partner_module_config?.module_definition?.type
        if (t) types.add(t)
      })
  })
  return [...types]
...
function handleOnboarded(result: any) {
  if (!result) return
  if (result.alreadyHasModule) {
    onboardToast.value = `${result.customer.full_name} heeft deze module al.`
  } else if (result.isExisting) {
    onboardToast.value = `Module toegevoegd aan ${result.customer.full_name}.`
  } else {
...
function saveEdit() {
  ...
  // Update the reactive customer object directly
  Object.assign(editingCustomer.value, {
    ...editForm.value,
...
// View as customer
function viewAsCustomer(customer: Customer) {
  // Update the currentCustomer in mock data to this customer
  ...
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">{{ customers.length }} klanten totaal</p>
      </div>
            ...
            >
              ...
              <td class="px-6 py-3.5">
                <div class="flex items-center gap-1.5">
                  <span
                    v-for="mType in getCustomerModuleTypes(customer.id)"
                    :key="mType"
                    class="flex h-6 w-6 items-center justify-center rounded-md"
                    :class="mType === 'solar' ? 'bg-amber-50 text-amber-600' : mType === 'heat_pump' ? 'bg-rose-50 text-rose-600' : 'bg-sky-50 text-sky-600'"
                    :title="mType === 'solar' ? 'Zonnepanelen' : mType === 'heat_pump' ? 'Warmtepomp' : 'Laadpaal'"
                  >
                    <AppIcon
                      :name="mType === 'solar' ? 'solar' : mType === 'heat_pump' ? 'heat-pump' : 'ev-charger'"
                      :size="14"
                    />
                  </span>
                  <span v-if="getCustomerModuleCount(customer.id) === 0" class="text-xs text-gray-400">Geen</span>
                </div>
              </td>
              ...
              <td class="px-6 py-3.5">
                <div class="flex items-center justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                  ...
                  <button
                    class="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-medium hover:bg-blue-50 hover:text-blue-700"
                    :style="{ color: 'var(--brand-primary)' }"
                    @click="viewAsCustomer(customer)"
                  >
          ...
          <div class="relative w-full max-w-lg rounded-2xl bg-white shadow-xl overflow-hidden">
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <template v-if="!editSaved">
                ...
                <div class="rounded-xl border border-gray-100 bg-gray-50 p-3">
                  <p class="text-xs font-medium text-gray-400 mb-2">Actieve modules</p>
                  <div class="flex items-center gap-2">
                    <template v-for="mType in getCustomerModuleTypes(editingCustomer.id)" :key="mType">
                      <span
                        class="badge"
                        :class="mType === 'solar' ? 'badge--yellow' : mType === 'heat_pump' ? 'badge--red' : 'badge--blue'"
                      >
                        {{ mType === 'solar' ? 'Zonnepanelen' : mType === 'heat_pump' ? 'Warmtepomp' : 'Laadpaal' }}
                      </span>
                    </template>
                    <span v-if="getCustomerModuleCount(editingCustomer.id) === 0" class="text-xs text-gray-400">Geen actieve modules</span>
                  </div>
                </div>
              </template>
            </div>
            ...
            <div v-if="!editSaved" class="border-t border-gray-100 px-6 py-4 flex items-center justify-between">
              <button
                ...
                :style="{ color: 'var(--brand-primary)' }"
                @click="editModalOpen = false; viewAsCustomer(editingCustomer!)"
              >
                <AppIcon name="external" :size="14" />
                Bekijk klantportaal
              </button>
